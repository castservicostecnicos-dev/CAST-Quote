...
import { WorkOrder, Client, Technician, ItemRow, PhotoRecord, ServiceItem } from '../types';
...
export const WorkOrderModal: React.FC<WorkOrderModalProps> = ({
  ...
  const draftStorageKey = orderToEdit
    ? `cast_draft_workorder_${orderToEdit.id}`
    : 'cast_draft_workorder_new';
  ...
  const hasMeaningfulOrderChanges = React.useCallback((d: typeof currentFormData) => {
    if (!d) return false;
    if (d.serviceDescription && d.serviceDescription.trim().length > 0) return true;
    ...
    if (d.clientSignature || d.technicianSignature) return true;
  ...
  const {
    ...
    isReady: isFormReady,
    debounceMs: 1200,
    hasMeaningfulChanges: hasMeaningfulOrderChanges
  });
  ...
  const handleRestoreDraft = () => {
    ...
    if (d.serviceDescription !== undefined) setServiceDescription(d.serviceDescription);
    ...
    if (d.technicianSignature !== undefined) setTechnicianSignature(d.technicianSignature);
    if (d.technicianSignedAt !== undefined) setTechnicianSignedAt(d.technicianSignedAt);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const init = async () => {
        ...
        if (orderToEdit) {
          await initEditState(orderToEdit);
        } else {
          resetForm();
        }
        setIsFormReady(true);
  ...
  const insertItemBelow = (index: number, type: 'servico' | 'material' = 'material') => {
    ...
    if (items.length === 1 && !items[0].description.trim() && items[0].unit_price === 0) {
      ...
      setActiveItemIndex(0);
      setTimeout(() => {
        const el = document.getElementById('os-item-desc-0');
        if (el) el.focus();
      }, 50);
      return;
    }
    ...
    setTimeout(() => {
      const el = document.getElementById(`os-item-desc-${insertIndex}`);
      if (el) {
        el.focus();
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        date,
        status,
        service_description: effectiveDesc,
        address,
        subtotal,
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-2 sm:p-4 backdrop-blur-xs overflow-y-auto">
        <div className="relative w-full max-w-4xl max-h-[92vh] rounded-3xl bg-white shadow-2xl border border-slate-200 flex flex-col overflow-hidden my-auto">
          ...
          {showDraftBanner && savedDraft && (
            <div className="bg-emerald-50 border-b border-emerald-200 px-4 sm:px-6 py-2.5 flex flex-wrap items-center justify-between gap-2 text-xs text-emerald-950">
              <div className="flex items-center gap-2">
                <Sparkles className="w-4 h-4 text-emerald-600 flex-shrink-0" />
                <div>
                  <span className="font-bold text-emerald-950">Rascunho não finalizado encontrado:</span>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-1 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-2xs transition cursor-pointer active:scale-98"
                >
                  Restaurar Rascunho
                </button>
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-white hover:bg-emerald-100 text-emerald-800 border border-emerald-300 font-semibold text-xs transition cursor-pointer"
                >
                  Descartar
                </button>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-600 hover:text-emerald-800 transition cursor-pointer"
                    title="Cadastrar novo cliente rapidamente sem sair da OS"
                  >
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50/50 p-4 space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  <button
                    type="button"
                    id="btn-os-add-material"
                    onClick={() => insertItemBelow(activeItemIndex, 'material')}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-50 text-emerald-700 hover:bg-emerald-100 text-xs font-bold border border-emerald-200 transition cursor-pointer shadow-2xs"
                      ...
                      <React.Fragment key={index}>
                        <tr
                          className={`transition group ${
                            activeItemIndex === index ? 'bg-emerald-50/40' : 'hover:bg-white/90'
                          }`}
                          ...
                          <td className="py-2 px-1 align-middle text-center">
                            <div className="flex items-center justify-center gap-1">
                              <button
                                ...
                                className="p-1.5 rounded-lg text-emerald-600 hover:text-emerald-800 hover:bg-emerald-100 transition cursor-pointer font-bold"
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              <div className="flex flex-wrap items-center gap-1.5">
                                <button
                                  type="button"
                                  onClick={() => insertItemBelow(index, item.item_type)}
                                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-2xs transition cursor-pointer active:scale-98"
                ...
                {items.map((item, index) => (
                  <div
                    ...
                    className={`rounded-2xl border bg-white p-3.5 shadow-2xs space-y-3 transition ${
                      activeItemIndex === index ? 'border-emerald-400 ring-2 ring-emerald-100' : 'border-slate-200'
                    }`}
                  >
                    {/* Cabeçalho do Card */}
                    ...
                    <div className="pt-2.5 border-t border-slate-100 space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <button
                          type="button"
                          onClick={() => insertItemBelow(index, item.item_type)}
                          className="flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-xs transition cursor-pointer active:scale-98"
                        >
                          <Plus className="w-4 h-4" />
                          <span>+ Novo Item</span>
                        </button>
      ...
      {/* Datalist for fast item and service suggestions in OS */}
      <datalist id="os-item-datalist">
        {catalogServices
          .filter((cs) => cs.active)
          .map((cs) => (
      ...
      <QuickClientModal
        ...
        companyId={activeCompany?.id || orderToEdit?.company_id || 'comp-cast'}
